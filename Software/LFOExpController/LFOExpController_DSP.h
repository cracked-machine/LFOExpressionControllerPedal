...
enum DSP_EWAVEFORMS { SINE, TRIANGLE, RAMPUP, STEP4, SQUARE, PULSE } ;

// available waveforms. if you change this order make sure the last item matches one used  in PCINT2_vect condition!
DSP_EWAVEFORMS const DSP_Waveforms[] = { SINE, TRIANGLE, RAMPUP, STEP4, SQUARE, PULSE };
...
int DSP_GetAlgorithmicWaveData(DSP_EWAVEFORMS _Waveform) {
      ...
      switch(_Waveform) {
          // cases must exist in DSP_EWAVEFORMS and DSP_Waveforms[] OR they MUST be commented out to build successfully!
          case SINE:     
